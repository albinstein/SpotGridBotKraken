...
class GridBot:
    def __init__(self, params = {}):
        ...
        self.p_grids = params['p_grids']/100    # Porcentaje de distancia entre grids
    ...
    def is_available(self, symbol, price):
        ...
        "p":{"$gt":float(price * (1 - self.p_grids)), "$lt":float(price * (1+self.p_grids))},
        }
    ...
    def get_orders(self, symbol):
        ...
        range_orders = (midprice * (1 - self.n_grids*self.p_grids), midprice * (1 + self.n_grids*self.p_grids))
        ...
        for i in range(1, self.n_grids+1):
            ...
            if self.is_available(symbol, price):
                ...
                'type':'limit',
                'side':'buy',
                'amount':(self.s_grids * float(symbol_info['minNotional']))/price,
                'price':price,
                'params':{
            ...
            if self.is_available(symbol, price):
                ...
                'type':'limit',
                'side':'sell',
                'amount':((1 + (i*self.p_grids)) *self.s_grids * float(symbol_info['minNotional']))/price,
                'price':price,
                'params':{
                # "time_in_force":'PostOnly',
                    }
